        ...
        getLocationCoordinates(obj.Country, parseInt(obj.Count))
            .then(coordinates => {
                ...
                markersSept.addLayer(L.marker(coordinates)
                    .bindPopup(`<b>${obj.Country}</b><br>Sum: ${countryCounts[obj.Country]}`));
            })
            .catch(error => {
                console.error('Error:', error);
            });
...
d3.csv('../../Resources/Dec_clean_data_v2.csv').then(data => {
    ...
    // Create a new marker cluster group for December
    ...
    data.forEach(obj => {
        getLocationCoordinates(obj.Country, parseInt(obj.Count))
            .then(coordinates => {
                // Add a new marker to the marker cluster group, and bind a popup
                markersDec.addLayer(L.marker(coordinates)
                    .bindPopup(`<b>${obj.Country}</b><br>Sum: ${obj.Count}`)); // Use obj.Count instead of countryCounts[obj.Country]
            })
            .catch(error => {
                console.error('Error:', error);
            });
    ...
    // Add the marker cluster layer for December to the map
